...
- `*-guide.html`, `*-checklist.html`, `video-glossary.html`, `troubleshooting.html`: AdSense 심사와 사용자 탐색을 위한 영상 작업 상세 문서
...
- 영상 작업 가이드: 타임코드, 비트레이트, 클립 편집, 자막 싱크, 업로드 체크리스트, 압축, 썸네일, 용어집, 문제 해결 문서를 제공합니다.
...
- `sitemap.xml`과 `robots.txt`는 현재 `https://product-builder-ify.pages.dev/` 기준으로 작성되어 있습니다.
- 커스텀 도메인을 연결하면 `index.html`, `robots.txt`, `sitemap.xml`, `site.webmanifest`의 URL을 실제 도메인으로 교체하세요.
- Google Search Console에 사이트를 등록한 뒤 `https://product-builder-ify.pages.dev/sitemap.xml`을 제출하세요.
...
- AdSense 게시자 ID `ca-pub-8649619832836598`가 HTML head와 `ads.txt`에 적용되어 있습니다.
- AdSense 재검토 전에는 공개 URL에서 새 가이드 페이지와 `ads.txt`, `sitemap.xml`, `robots.txt`가 모두 200으로 열리는지 확인하세요.
